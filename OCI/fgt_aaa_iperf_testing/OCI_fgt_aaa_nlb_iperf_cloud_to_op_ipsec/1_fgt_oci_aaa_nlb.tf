...
module "hub_oci_config" {
  ...
  fgt_id           = "ad${each.key + 1}-fgt${each.key + 1}-ipsec"
  ...
  config_s2s = true
  s2s_peers  = local.hub_oci_s2s_peers[each.key]

  vcn_spoke_cidrs = concat(local.hub_oci_spokes_cidrs, [local.hub_op_vcn_cidr])

  config_extra = templatefile("./templates/fgt_cpu_affinity.conf", { "port" = "port2" })
}
...
module "hub_oci" {
  ...
  instance_shape = local.hub_oci_shape
  fgt_image_ids  = local.hub_oci_image_ids
